...
// NOTE: Flip integration is temporarily disabled until Flip Production API is ready.
// To re-enable, uncomment the code below and uncomment the related blocks in:
//   - controllers/order.go  → processPaymentGateway (Flip block)
//   - controllers/order.go  → PaymentCallback handler body
//   - routes/routes.go      → POST /orders/payment-callback
...
// 	log.Printf("[Flip-Debug] Authorization: Basic (API Key present)")
